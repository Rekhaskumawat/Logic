...
int main()
{
    ...
    char * message = "Server : Jay Ganesh \n";
    ...
    // Step 3 : mark the socket as a active socket (in listining mode)

    iRet = listen(ServerSocket , 11 );

    if(iRet == -1)
    {
        printf("Uable to covert socket in listen mode\n");
        return -1 ;
    }
    ...
    if(ClientSocketFD == -1)
    {
        printf("Uable to accpet reuest from client \n");
        close(ServerSocket);
        return -1;
    }
